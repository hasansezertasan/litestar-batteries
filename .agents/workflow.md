...
> Canonical commands for this repo. No `make`/`just` wrapper exists yet — these are the `uv`-native
> entrypoints. If a `Makefile`/`justfile` is added later, update these to the wrapper targets.

### Setup
```bash
uv sync            # install deps + dev deps into the project venv
```
...
uv run mypy                  # type check (or: uv run pyright)
```
...
> (see `tech-stack.md`). CI runs these same commands across Python 3.10 & 3.14.
